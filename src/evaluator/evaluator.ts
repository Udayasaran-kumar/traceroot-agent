import type {
  ...
  const EXPECTED_FILE = "src/orders/service.ts";
  
  const EXPECTED_FUNCTION = "createOrder";
  ...
  function scoreRootCause(
    ...
    return {
      score: 0,
      reason: "Does not identify the expected root cause.",
    };
  }
  ...
  function scoreVerification(
    ...
    if (verification.status === "confirmed") {
      return {
        score: 1,
        reason: "Verification reports the root cause as confirmed.",
      };
    }
  ...
  function scoreEvidence(
    ...
    return {
      score: 1,
      reason: `Provided ${evidence.length} supporting evidence item(s).`,
    };
  }
  ...
  function evaluateBaseline(
    ...
    const location = scoreBaselineLocation();
  ...
  export async function evaluateIncident(
    ...
    const agentResult =
      await investigateIncident(incidentDirectory);
